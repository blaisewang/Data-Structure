...
template<class T>
struct wEdge { // vertex and weight pair
    int vertex;
    T weight;
...
public:
    linkedWDigraph(int numberOfVertices = 0) {  // Constructor.
    ...
    bool existsEdge(int i, int j) const {  // Return true iff (i,j) is an edge.
        return !(i < 1 || j < 1 || i > n || j > n || aList[i].indexOf(wEdge<T>(j)) == -1);
    }

    void insertEdge(edge<T> *theEdge) {  // Insert an edge.
        ...
        if (aList[v1].indexOf(wEdge<T>(v2)) == -1) {  // new edge
            aList[v1].insert(0, wEdge<T>(v2, theEdge->weight()));
            e++;
        }
    }

    void insertEdgeNoCheck(edge<T> *theEdge) {  // Insert an edge. No validation checks performed.
        aList[theEdge->vertex1()].insert(0, wEdge<T>(theEdge->vertex2(), theEdge->weight()));
        e++;
    }
    ...
    int outDegree(int theVertex) const {  // Return out-degree of vertex theVertex.
    ...
    public:
        ...
        int next(T &theWeight) {    // Return next vertex if any. Return 0 if no next vertex.
    ...
    myIterator *iterator(int theVertex) {    // Return iterator for vertex theVertex.
        checkVertex(theVertex);
        return new myIterator(aList[theVertex].firstNode);
    }

    void output(ostream &out) const {   // Output the adjacency matrix.
        for (int i = 1; i <= n; i++) {
            out << aList[i] << endl;
        }
    }
    ...
    void dfs(int v, int reach[], int label) const {
        ...
        while (!s.empty()) {
            ...
            currentNode = aList[w].firstNode;

            // mark all unreached vertices adjacent from w
            while (currentNode) {
                // visit an adjacent vertex of w
                if (reach[currentNode->element.vertex] == 0) {
                    // currentNode->element.vertex is an unreached vertex
                    s.push(currentNode->element.vertex);
                    reach[currentNode->element.vertex] = label;    // mark reached
                }
                currentNode = currentNode->next;
            }
        }
    ...
    adjacencyWDigraph<T> &convertToMatrix() {    // return linkedWDigraph Type pointer
        adjacencyWDigraph<T> *a = new adjacencyWDigraph<T>(n);
        chainNode<wEdge<T> > *currentNode;

        //	traversal linked list to get all edges
        for (int i = 1; i <= n; i++) {
            currentNode = aList[i].firstNode;
            while (currentNode) {
                a->insertEdge(new weightedEdge<int>(i, currentNode->element.vertex, currentNode->element.weight));
                currentNode = currentNode->next;
            }
        }

...
public:
    ...
    int inDegree(int theVertex) const {  // Return in-degree of vertex theVertex.
        checkVertex(theVertex);
    ...
    void bfs(int v, int reach[], int label) const {    // Breadth-first search. reach[i] is set to label for
        ...
        while (!q.empty()) {
            ...
            for (int u = 1; u <= n; u++) {
                ...
                if (a[w][u] != noEdge && reach[u] == 0) {       // u is an unreached vertex
                    q.push(u);
    ...
    void dfs(int v, int reach[], int label) const {
        ...
        while (!s.empty()) {
            ...
            for (int u = n; u >= 1; u--) {
                // visit an adjacent vertex of w
                if (a[w][u] != noEdge && !reach[u]) {    // u is an unreached vertex
                    s.push(u);
                    reach[u] = label; // mark reached
                }
            }
    ...
    linkedWDigraph<T> &convertToList() {    // return linkedWDigraph Type pointer
